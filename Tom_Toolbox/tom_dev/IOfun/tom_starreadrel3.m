function [star_st,Header]=tom_starreadrel3(filename,outputFlavour,NumOfLines2chk)
...
while 1
    ...
    if rel3loop==1 && isempty(findstr(tmpLine,'data_'))==0
        tmpLine=fgetl(fid);
        numOfTotalLines=numOfTotalLines+1;
        while isempty(findstr(tmpLine,'data_'))==1
            tmpLine=fgetl(fid);
            numOfTotalLines=numOfTotalLines+1;
        end
        rel3loop=2;
    ...
    if ( (tmpLine(1)=='_') || strcmp(deblank(tmpLine),'loop_') || isempty(findstr(tmpLine,'data_'))==0 || isempty(findstr(tmpLine,'# v'))==0)
