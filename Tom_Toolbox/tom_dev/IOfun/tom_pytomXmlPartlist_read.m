function [st,Align]=tom_pytomXmlPartlist_read(filename,atribute,isExMaxJop)
...
%   read image
%
%   [st,Align]=tom_pytomXmlPartlist_read('ExMaxJob-1.xml');
%   %4 use with tom_av3_stackbrowser 
%   save('alignList.mat','Align');
%   avg=tom_av3_average(Align); figure; tom_dspcub(avg);
% 
%REFERENCES
%
%SEE ALSO
...
if (isExMaxJop==1 || strcmp(tmpStruct.Name,'ParticleList')~=1 )
...
for i=1:length(idx)
    ...
    st(i).WedgeParam=[str2double(tmpStruct.Children(idx(i)).Children(8).Children(2).Attributes(1).Value)  str2double(tmpStruct.Children(idx(i)).Children(8).Children(2).Attributes(2).Value)];
...
for i=1:length(st)
    ...
    Align(i).Wedge.Angle1=st(i).WedgeParam(1);
    Align(i).Wedge.Angle2=st(i).WedgeParam(2);
...
if (strcmp(theStruct(1).Name,'#comment'))
    tmp=theStruct(2);
    clear('theStruct');
    theStruct=tmp;
end;
