function [vol_var,vol_avg,vol_var_even,vol_var_odd]=tom_av3_calc_variance(basepath,extension,num_of_models,mask,filter_kernel,outputlevel,outputFolder)
%tom_av3_calc_variance calculates a 3d variance Map 
%
%   [vol_var vol_avg vol_var_even vol_var_odd]=tom_av3_calc_variance(basepath,extension,num_of_models,mask,filter_kernel,outputlevel,outputFolder)
%
...
if (tom_ismrcfile(f_list{1}))
    spi_flag=3;
    im_tmp=tom_mrcread(f_list{1});
end;
...
if (nargin <7)
   outputFolder='var_out';
end;

if (strcmp(num_of_models,'all_you_can') || isnumeric(num_of_models)==0)
...
try
    ...
    tom_emwrite([outputFolder filesep 'var_vol.em'],vol_var);
    tom_emwrite([outputFolder filesep 'var_even.em'],vol_var_even);
    tom_emwrite([outputFolder filesep 'var_odd.em'],vol_var_odd);
    tom_emwrite([outputFolder filesep 'avg.em'],vol_avg);
catch ME
    disp('Cannot write variances!');
    disp(ME.message);
end;
unix(['rm -R ' a_base '/diffs']);
...
%         end;
        on_disk=0;
        if (num_of_models>10000)
            disp('Stack size > 10000 calculating var on disk!')
            on_disk=1;
        end;
