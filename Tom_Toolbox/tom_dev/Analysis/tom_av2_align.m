...
if (demo~=0)
    demo_al(ref,'ref',0);
    demo_al(tom_filter(im,demo_fcer),'im',0);
end;
...
for i=1:num_of_iterations
   ...
   % ccf_rot=tom_corr(im_polar,ref_polar,'norm',tom_cart2polar(mask_im));
    ...
    if (demo~=0)
        ...
        demo_al(tom_filter(im_polar,demo_fcer),'im_polar',0);
        demo_al(ccf_rot,'ccf_rot',ccc_pos_rot);
        demo_al(ref,'ref_rot',0);
        demo_al(tom_filter(im_rot,demo_fcer),'im_rot',0);
    ...
    im=aligned_part_sum;
    if (demo~=0)
        demo_al(tom_filter(aligned_part_sum,demo_fcer),'im_alg',i);
    end;
...
if (demo~=0)
    demo_al(tom_filter(aligned_part_sum,3),'im_alg_final');
end;
 
% demo options
function demo_al(im,flag,val)

mid=floor(size(im))./2+1;
sctFactor=[1 1.03 1.26 1.26];
splR=6;
...
if (strcmp(flag,'im'))
    g=subplot(splR,3,1); tom_imagesc(im,'noinfo');title('particle');axis off;
    p = get(g,'position');
    set(g,'position',p.*sctFactor);
    drawnow;
end;


if (strcmp(flag,'ref'))
    g=subplot(splR,3,3); tom_imagesc(im,'noinfo');
...
if (strcmp(flag,'im_polar'))
    g=subplot(splR,3,4); tom_imagesc(im,'noinfo');title('particle polar');axis off;
    p = get(g,'position');
    set(g,'position',p.*sctFactor);
    drawnow;
end;
...
if (strcmp(flag,'ref_polar'))
    g=subplot(splR,3,6); tom_imagesc(im,'noinfo');
...
if (strcmp(flag,'ccf_rot'))
    g=subplot(splR,3,8); tom_imagesc(im,'noinfo');title('xcf polar');axis off;
    hold on; plot(val(1),val(2),'r+'); plot(mid(1),mid(2),'g+');  hold off;
    p = get(g,'position');
     set(g,'position',p.*sctFactor);
    drawnow;
end;
...
if (strcmp(flag,'im_rot'))
    g=subplot(splR,3,10); tom_imagesc(im,'noinfo');title('particle rotated');axis off;
    p = get(g,'position');
     set(g,'position',p.*sctFactor);
    drawnow;
end;
...
if (strcmp(flag,'ref_rot'))
    g=subplot(splR,3,12); tom_imagesc(im,'noinfo');
    title('reference');axis off;
    p = get(g,'position');
     set(g,'position',p.*sctFactor);
    drawnow;
...
if (strcmp(flag,'ccf_trans'))
    g=subplot(splR,3,14); tom_imagesc(im,'noinfo');title('xcf trans');axis off;
    hold on; plot(val(1),val(2),'r+');plot(mid(1),mid(2),'g+');  hold off;
    p = get(g,'position');
     set(g,'position',p.*sctFactor);
    drawnow;
...
if (splR>7)
    ...
    if (strcmp(flag,'new_ref_rotated_trans'))
        g=subplot(splR,3,21); tom_imagesc(im,'noinfo');
