...
[filename, pathname] = uigetfile( ...
    {'*.em;*.mrc', 'All TOM/Mrc Files (*.em,*.mrc)'; ...
    '*.*',                   'All Files (*.*)'}, ...
    'Pick an image file');
...
if filename==0
    ...
    set(handles.gui_file_slider,'Value',val);
    if (num_of_files>1)
        set(handles.gui_file_slider,'Visible','on');
        set(handles.gui_file_slider,'SliderStep',[(1./(num_of_files-1)) (1./(num_of_files-1))]);
    else
        set(handles.gui_file_slider,'Visible','off');
    end;
...
if (tom_isemfile([handles.pathname handles.filename])==1)
    ...
    if header.Header.Size(1)<513
     ...
     in=tom_mrcread([pathname filename]);
end;
...
function handles=reload_it_man(handles,pathname,filename,coming_from_slider)
    ...
    if (tom_isemfile([pathname filename]))
        ...
        in=tom_mrcread([pathname filename]);
        header.Header.Objectpixelsize=0;
        header.Header.Magnification=0;
        header.Header.Size=in.Header.Size;
    ...
    if header.Header.Objectpixelsize==0 && coming_from_slider==0
        ...
        handles.magnification=str2num(get(handles.gui_magnification,'String'));
        handles.pixelsize=str2num(get(handles.gui_pixelsize,'String'));
        handles.voltag=str2num(get(handles.gui_voltage,'String')) .*1000;
        handles.cs=str2num(get(handles.gui_cs,'String'));
        handles.cc=str2num(get(handles.gui_cc,'String'));
...
%    
    ...
    function handles=create_ps(handles)
        ...
        if handles.region(1)<handles.image_fullsize(1)
            ...
            if (tom_isemfile([handles.pathname handles.filename]))
                in=tom_emreadc([handles.pathname handles.filename]);
            else
                in=tom_mrcread([handles.pathname handles.filename]);
            end;
