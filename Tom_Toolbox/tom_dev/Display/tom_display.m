function tmpMat=tom_display(in,flag)
...
tmpMean='';
if (isstruct(in))
    if (isfield(in,'D') && isfield(in,'Mean'))
        Zsize=size(in.D,2);
        tmpMean=reshape(in.Mean,[in.OrgSize(1) in.OrgSize(2)]);
        tmpMat=reshape(in.D,[in.OrgSize(1) in.OrgSize(2) Zsize]);
        tmpMat=cat(3,tmpMean,tmpMat);
        for i=1:size(tmpMat,3)
            tmpMat(:,:,i)=tom_norm(tmpMat(:,:,i),'mean0+1std');
        end;
        figure; tom_dspcub(tmpMat);
        return;
    end;
