function part_ord=tom_tree2chimera(tree,volume_list,thresholds,spacing_fact,map4Score,colormap,tmp_dir,spacing4Vols,ColorThreshold,hideDust)
...
%   volume_list    (opt.) as  mat-cell  ...file order !!   
...
%   colormap       ([1 0 0]) colormap(s) for the models
%   tmp_dir        ('./xxx_tmp4dend_xxx') directrory where tmp files are stored  
%   spacing4Vols   ('volSize') or size in pixels   
%   ColorThreshold ('off') enter a numeric Value 2 switch on
%   hideDust       ('') enter value from chimera to activate  
%
%  OUTPUT
...
%  end;
%  [cc vol_list] = tom_av3_ccmat('4testXX/v_','em',6);
%  tree = linkage(squareform(1-cc),'single');
%  tom_tree2chimera(tree,vol_list,0.28,2.1,'quadr',[1 0 0],'myOutput4Dendro','volSize',0.5);
%  dendrogram(tree);
%  
%
%
...
if (strcmp(spacing4Vols,'volSize'))
    off4dln=round(sz(1)/2);
else
    off4dln=spacing4Vols./2;
end;
off4Labels=round(sz(1)/2);
...
for i=1:size(tree,1)
    ...
    [coord4label,label_c]=calc_label_coord(coord4label,root_coord,first_row_y,scaleX,scaleY,off4Labels,label_c);
...
[theGroups,~,cmap]=genColorLookUp(tree,ColorThreshold);
gen_build_file(tmp_dir,co4dendlin,theGroups,cmap);
gen_build_file_label(tmp_dir,coord4label,part_ord,off4Labels);

all_bild{1}=[tmp_dir '/lines4dend.bild'];
all_bild{2}=[tmp_dir '/labels.bild'];

tom_chimera_volumes('show.py',volume_list_all,'',thresholds,colormap,'',coord_list_all,hideDust,all_bild);
...
function gen_build_file(tmp_dir,co4dendlin,theGroups,cmap)
...
for i=1:(size(co4dendlin,2)/3)
    if (isempty(theGroups))
        ...
        cmpIdx=theGroups(n);
        if (cmpIdx==0)
           cmpIdx=max(theGroups)+1; 
        end;
        actColour=cmap(cmpIdx,:);
    end;
    
    fprintf(fid,'.color %f %f %f\n',actColour(1),actColour(2),actColour(3));
    zz=zz+1;
