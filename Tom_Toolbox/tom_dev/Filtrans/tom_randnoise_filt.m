function out=tom_randnoise_filt(in,mask,outputname,grow_rate,sdRange,blackdust,whitedust)
...
if (grow_rate==0)
    ...
    outsmall=find(in(outmask)<-sdRange);
    %insmall=find(in(inmask)<-1.5);
    %allsmall=find(in<-.5);
    outlarge=find(in(outmask)>sdRange);
