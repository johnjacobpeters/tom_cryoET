function [average, unwei_av,wei] = av3_average_exact(motl, particlefilename,wedgelist,threshold,iclass)
...
%check that motl(5,:) contains indices in wedgelist
...
lowpMask =tom_spheremask(ones(size(average)),floor(size(average,1)./2)-4,2);
wei = tom_filter(wei,2);% filter to avoid artefacts for badly sampled data
ii = find(wei < 1);wei(ii)=1;
wei = 1./wei;rind = find(wei > 100000);wei(rind) = 0;% take care for inf
wei=wei.*lowpMask;
unwei_av = average;
%average = real(tom_ifourier(ifftshift(fftshift(tom_fourier(average)).*wei) ));
average=tom_apply_weight_function(average,wei);
