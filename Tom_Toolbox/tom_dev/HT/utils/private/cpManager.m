...
%   $Revision: 1.1.6.5 $  $Date: 2006/12/10 20:03:32 $
...
keyPressCallbackId = iptaddcallback(hFig,'KeyPressFcn',@manageKeyPress);
...
pairsChanged
  ...
  %-----------------------------------
  function addInputPoint(obj,varargin) %#ok varargin needed by HG caller
  ...
  %----------------------------------
  function addBasePoint(obj,varargin) %#ok varargin needed by HG caller
  ...
  %----------------------------------------------
  function addInputPointPredictBase(obj,varargin) %#ok varargin needed by HG caller
  ...
  %----------------------------------------------
  function addBasePointPredictInput(obj,varargin) %#ok varargin needed by HG caller
  ...
  function createPredictedPoint(x,y,pairId,predictBase,createFcn)
    ...
    % clicked on when a point is made active. This information is needed for
    % arrow key movement of control points.
	if predictBase
  ...
  %--------------------------------------
  function changeActivePair(src,varargin) %#ok varagin needed by HG caller
  
    clickedPoint = src;
    pointAPI = iptgetapi(clickedPoint);
    
    pairId = pointAPI.getPairId();
  ...
  function pairsChanged
    ...
    % Update needToSave based on whether there are any input or base points
     if ~isempty(inputBasePairs)
         needToSave = true;
     else
         needToSave = false;
     end    
  ...
  function recycleActivePair
    ...
    %------------------------------
    function manageKeyPress(src,ed)
  ...
  %----------------------------------
  function deleteActivePair(varargin) %#ok varargin needed by HG caller
  ...
  %----------------------------------------
  function deleteActiveInputPoint(varargin) %#ok varargin needed by HG caller
  ...
  %---------------------------------------
  function deleteActiveBasePoint(varargin) %#ok varargin needed by HG caller
