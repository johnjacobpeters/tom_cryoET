...
%   $Revision: 1.1.8.3 $  $Date: 2007/01/15 14:37:58 $
  ...
  p = figparams;
  % get the size of the figure decorations
  % e.g. title bar, menu bars, borders
  horizontal_decorations = p.RightDecoration + p.LeftDecoration;
  vertical_decorations     = p.BottomDecoration + p.TopDecoration;
  
  calculateDimensions % to initialize dimensions
  ...
  if warn_about_mag
      ...
      warning(wid,...
              'Image is too big to fit on screen; displaying at %d%%',...
  ...
  function calculateDimensions
     ...
     is_width_bigger_than_screen = ...
         (new_fig_width + horizontal_decorations) > screen_width;
     is_height_bigger_than_screen = ...
         (new_fig_height + vertical_decorations) > screen_height;
  end
  ...
  function w = getGutterWidth
     ...
     if isBorderTight
         ...
         w = 2*ceil(p.YLabelWidth); % make it symmetric for balance
     end
  ...
  function h = getGutterHeight
     ...
     if isBorderTight
         ...
         h  = ceil(p.XLabelHeight + p.AxesTitleHeight);
     end
