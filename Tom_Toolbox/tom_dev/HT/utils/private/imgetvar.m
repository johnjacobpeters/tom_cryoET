...
%   $Revision: 1.1.8.3 $  $Date: 2005/11/15 01:03:41 $
  ...
  top_margin = 10;
  spacing = 5;
  default_panel_height = fig_height - bottom_margin - top_margin;
  default_panel_width = fig_width - left_margin -right_margin;
  default_panel_position = [left_margin, bottom_margin,...
  ...
  function hPanel = createFilterMenu
    ...
    image_type_str = {'All (M-by-N, M-by-N-by-3)',...
  ...
  %------------------------------------------------  
  function doButtonPress(src,evt)
  ...
  function status = getVars
    ...
    if isempty(list_str)
      ...
      if isempty(all_str)
        error_str = sprintf('There are no variables containing image');
        error_str = sprintf('%s data in the workspace.',error_str);
        
      else
        error_str = sprintf('You have not selected a variable from the workspace.');
      end
      errordlg(error_str);
      status = FAILURE;
      return;
  ...
  %---------------------------------------------------------
  function callImChoosecmap(src,evt,hImportFig,hEditCMap)
    dynamic_behavior = true;
    imchoosecmap(hImportFig,dynamic_behavior,hEditCMap)
  end
...
function [out, eval_passed] = evaluateVariable(var_name)
  ...
  eval_passed = true;
  out = evalin('base',sprintf('%s;',var_name),'''errorOccurred'';');
  if strcmp(out,'errorOccurred')
    error_str = lasterr;
    errordlg(error_str)
    eval_passed = false;
    return;
