...
%   ctf_corr           ('none') flag for "on-the-fly" ctf correction (fitting has to be done before)
...
%
% read emanBoxes and create a stack of images
% 
% plCell{1}='e2Box/img_*.box';
% plCell{2}='/fs/pool/pool-nickell3/fb/4Tueb/test_mult/img_';
% plCell{3}='.mrc'; 
% plCell{3}=0; %keep org box filename
% plCell{4}=[0 0]; %offset
%
% stack=tom_av2_xmipp_picklist2stack(plCell,'','',280,'gradient&mean0+1std',1,[64 64]);
%
%REFERENCES
%
%SEE ALSO
...
if isempty(poolobj)
    ...
    myNumOfProc = poolobj.NumWorkers;
end;


my_disp(['Running on: ' num2str(myNumOfProc) ' processors'],'info: ',1,log_file_name,1);
...
[align2dReOrg,lenAlign2dReorg]=reorgAlign2d(align2d,align_flag,log_file_name);
...
%for i=1:length(align2dReOrg)
   [tmpstack{i},tmppos{i}]=processPickList(align2dReOrg{i},outfilestruct,radius,norm_str,grad_filt,mask_backGround,invert,start,mask,split_floder,rad_fact,taper_flag,on_error,randperm,ctf_corr,field_corr,xray,mynargout,log_file_name,dspFlag);
end;
...
if (isempty(outfilestruct(1).fid)==0 )
     ...
     idxNewLine=findstr(b,char(10));
     if (length(idxNewLine)>1)
         b=b(idxNewLine(end-1):end);
     end;
...
function [tmpstack,posVect]=processPickList(align2dReOrg,outfilestruct,radius,norm_str,grad_filt,mask_backGround,invert,start,mask,split_floder,rad_fact,taper_flag,on_error,randperm,ctf_corr,field_corr,xray,valback,log_file_name,dspFlagIn)
...
function [list,lenTotal]=reorgAlign2d(align2d,alignFlag,log_file_name)
zz=1;
my_disp('Reorganizing Picklist','info: ',1,log_file_name);
...
lenTotal=part_count;
my_disp(['ReOrganized PickList==> total length of: ' num2str(lenTotal)],'info: ',1,log_file_name);
my_disp(['ReOrganized PickList==> number of micrographs: ' num2str(length(u_filenames))],'info: ',1,log_file_name);
...
ftype=tom_determine_file_type2(img_name);

if (strcmp(ctf_corr,'none')==0 || (isempty(GainAndDark{1})==0 && isempty(GainAndDark{2})) || strcmp(ftype,'em')==0 )
    if (strcmp(img_buffer.name,img_name)==0)
        try
            %my_disp(['Reading: ' img_name],'info: ',0,log_file_name,1);
            %img_buffer.img=tom_emreadc(img_name);
            img_buffer.img=readImage(img_name);
...
if (strcmp(ctf_corr,'none')==0)
    ...
    if (size(img,1)~=size(img,2) )
        my_disp('no quad image found!','info: ',0,log_file_name,1);
        if (size(img,1)>size(img,2))
            stopX=size(img,2);
            stopY=size(img,2);
            startX=size(img,1)-size(img,2)+1;
            startY=1;
        else
            stopX=size(img,1);
            stopY=size(img,1);
            startY=size(img,2)-size(img,1)+1;
            startX=1;
        end;
        imgCTmp=zeros(size(img),'single');
        imgCTmp(1:stopX,1:stopY)=single(tom_correct_for_ctf_and_mtf_new(img(1:stopX,1:stopY),st_out.Fit,ctf_corr,cutSz,''));
        imgCTmp(startX:end,startY:end)=single(tom_correct_for_ctf_and_mtf_new(img(startX:end,startY:end),st_out.Fit,ctf_corr,cutSz,''));
        img=imgCTmp;
        clear('imgCTmp');
    else
...
if (isnumeric(particleBuffer))
    ...
    particleBuffer.stack=zeros(sz_part(1),sz_part(2),lenList,'single');
...
particleBuffer.stack(:,:,particleBuffer.count)=single(particle);
...
if (lenAlign>200000 && strcmp(split_floder,'off'))
    warning('picklist bigger > 400000');
    disp('consider using split_flag with 200000');
end;
...
try
    ...
    if (iscell(picklist))
        align2d=tom_av2_eman_box2align(picklist{1},picklist{2},picklist{3},picklist{4});
    end;
...
if (strcmp(field_corr,'none')==0)
    for iGAD=1:length(field_corr)
        try
            ...
            tmpImg=readImage(field_corr{iGAD});
        catch Me
...
try
    ...
    if (size(particle.Value,1) > (radius*rad_fact*2))
        particle.Value=particle.Value(1:radius*rad_fact*2,:);
    end;
    if (size(particle.Value,2) > (radius*rad_fact*2))
        particle.Value=particle.Value(:,1:radius*rad_fact*2);
    end;
...
function img=readImage(name)

 type=tom_determine_file_type2(name);
 
 if (strcmp(type,'em'))
     img=tom_emreadc(name);
 end;
 
 if (strcmp(type,'mrc'))
     img=tom_mrcread(name);
 end;

 if (strcmp(type,'spider'))
     img=tom_spiderread(name);
 end;
