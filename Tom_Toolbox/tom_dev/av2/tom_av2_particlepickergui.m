...
if size(varargin,1) == 0
    ...
    if (isstruct(varargin{1})==0)
        if (exist(varargin{1},'dir')>0)
            ...
            [~,~,tmpExt]=fileparts(varargin{1});
            if (strcmp(tmpExt,'.mat'))
                tt=load(varargin{1});
                pl_tmp=tt.align2d;
                storage_av2_particlepicker.path=fileparts(pl_tmp(1,1).filename);
                pickList_in_flag=1;
...
if (pickList_in_flag==0)
    if (useWildCard==1)
        ...
        storage_av2_particlepicker.path=[];
        [foldImg,bb,cc]=fileparts(varargin{1});
        if (isempty(foldImg))
            foldImg=uigetdir();
            wkAbs=[foldImg filesep bb cc];
        else
            wkAbs=varargin{1};
        end;
        storage_av2_particlepicker.path=foldImg;
        d=dir(wkAbs);
        for i=1:length(d)
            storage_av2_particlepicker.dircell{i}=d(i).name;
        ...
        for k = 1:size(storage_av2_particlepicker.points(i).points,2)
            ...
            if strmatch(storage_av2_particlepicker.align(1,j).class,classname,'exact');
                ...
                if x <= radius*2 | y <= radius*2 | x > imagesize(1)-radius*2-1 | y > imagesize(2)-radius*2-1
                    ...
                    if (tom_isemfile([storage_av2_particlepicker.align(1,j).filename]))
...
if ischar(filename)
    ...
    if (tom_isemfile([pathname '/' filename]))
        ...
        im = tom_mrcread([pathname '/' filename]);
    end;
...
for i = 1:files
    if dirlist(i).isdir == 0
        if isempty(strmatch(dirlist(i).name,dircell))
            if tom_isemfile([directory '/' dirlist(i).name]) == 1 || tom_ismrcfile([directory '/' dirlist(i).name]) ==1
...
if storage_av2_particlepicker.flagcell{number} == 0
    ...
    if storage_av2_particlepicker.imagenumber ~= number || forcereload == 1
       try
           if (tom_isemfile([storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{number}])) 
               ...
               storage_av2_particlepicker.resampleval=1;
               im = tom_mrcread([storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{number}]);
           end;
...
if storage_av2_particlepicker.flagcell{number} == 0
    ...
    if storage_av2_particlepicker.imagenumber_high ~= number || forcereload == 1
       ...
       if (tom_isemfile()) 
           ...
           im=tom_mrcread([storage_av2_particlepicker.path_high '/' storage_av2_particlepicker.dircell{number}]); 
       end;
...
%     end
    ...
    if x1 <= radius*2 | y1 <= radius*2 | x1 > imagesize(1)-radius*2-1 | y1 > imagesize(2)-radius*2-1
        ...
        %cut out particle, this will give a non quadratic matrix
        if (tom_isemfile([storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{storage_av2_particlepicker.imagenumber}]))
        ...
        if (tom_isemfile([storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{storage_av2_particlepicker.imagenumber}]))
            ...
            try
                img=getimage(gca)';
                pos=[x1-2*radius y1-2*radius 1];
                sz=[4*radius-1 4*radius-1 0];
                part_box.Value = tom_cut_out(img,pos,sz);
               
            catch Me
                part_box.Value=ones(sz);
            end;
        end;
