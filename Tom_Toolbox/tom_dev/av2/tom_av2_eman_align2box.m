function tom_av2_eman_align2box(f_align,box_folder,box_size,funny_number,doRound,postFix)
...
%   
%   tom_av2_eman_align2box('picklist_high.mat','box_files_low',[128 128],-3,1,'_carbrm');
%
%REFERENCES
%
%SEE ALSO
...
for i=1:length(name_list)
    ...
    for ii=1:length(idx)
        ...
        if (doRound)
            coords_x(ii)=round(coords_x(ii));
            coords_y(ii)=round(coords_y(ii));
            if (coords_x(ii)<1)
                coords_x(ii)=1;
            end;
            if (coords_y(ii)<1)
    ...
    write_box([box_folder '/' b postFix '.box'],coords_x,coords_y,box_size,funny_number);
