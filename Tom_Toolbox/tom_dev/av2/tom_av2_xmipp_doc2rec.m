function vol=tom_av2_xmipp_doc2rec(path_doc,sym,discard_p,max_res,np,outputfilename,verbose,outputfolder,clean_up,mpi_prefix,ref_angles,add_param_cl_av,add_param_rec,fsc_param,rec_flav)
...
%   rec_flav            ('four') flavour 4 reconstruction 'art' or 'four' implemented        
%
%  OUTPUT
%
...
for i=1:sp_nr
    ...
    program_name=get_abs_prog_name(rec_flav,np);
...
if (sp_nr>1)
    ...
    if (strcmp(rec_flav,'four'))
        ...
        parameters=['-ref ' outputfolder '/vols/rec_sp1.spi.vol' ' -i ' outputfolder '/vols/rec_sp2.spi.vol' ' -sam ' num2str(fsc_param(1))  ' -max_sam ' num2str(fsc_param(2)) ];
    end;
    ...
    if (strcmp(rec_flav,'four'))
       ...
       unix(['mv ' outputfolder '/vols/rec_sp2.spi.vol.frc ' output_str ]);  
    end;
end;
...
if (nargout > 0 || isempty(outputfilename)==0)
    name2Read=[outputfolder '/vols/rec.spi'];
    if (strcmp(rec_flav,'art'))
        name2Read=[name2Read '.vol'];
    end;
    vol=tom_spiderread(name2Read);
    vol=vol.Value;
end;
...
if (strcmp(prog,'art'))
    if (np == 1)
        [a name_abs]=unix('which xmipp_reconstruct_art');
    else
        [a name_abs]=unix('which xmipp_mpi_reconstruct_art');
    end;
end;
