function align2d=tom_av2_eman_box2align(f_box,base_p_imges,ext_in,keepBoxFileName,offset)
...
%   keepBoxFileName  (0)  use 1 to switch on
%   offset           ([0 0]) offset for non quad images or whatever  
%
%  EXAMPLE
%     
%  
...
%
% (4)
% micrographs and box have a funny naming sceme but differ only by extension: test_1.mrc,
% test_44xx55_1.box test_44xx55_1.em
%
% align2d=tom_av2_eman_box2align('hstpp_eman_*.box','/fs/pool/pool-tpp/TPP_tom_acquisition/TPP_240709_clean_2/792009/high/','.mrc',1);    
%
...
% K2-counting  
% 
% align2d=tom_av2_eman_box2align('hstpp_eman_*.box','/fs/pool/pool-tpp/TPP_tom_acquisition/TPP_240709_clean_2/792009/high/','.em',1,[64 0]); 
%
...
for i=1:length(st)
    ...
    for ii=1:size(tmp,1)
        ...
        if (keepBoxFileName)
            [~,tmpBoxFileName]=fileparts(st(i).name);
            align2d(1,zz).filename=[base_p_imges tmpBoxFileName ext_in];
        else
        ...
        align2d(1,zz).position.x=tmp(ii,1)+round(tmp(ii,3)./2)+offset(1);
        align2d(1,zz).position.y=tmp(ii,2)+round(tmp(ii,3)./2)+offset(2);
