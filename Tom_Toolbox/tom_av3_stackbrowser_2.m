function varargout = tom_av3_stackbrowser_2(varargin)
%TOM_AV3_STACKBROWSER_2 is a GUI for browsing 3D particle stacks
%
%   varargout = tom_av3_stackbrowser_2(varargin)
%
%PARAMETERS
%
%  INPUT
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @tom_av3_stackbrowser_2_OpeningFcn, ...
                   'gui_OutputFcn',  @tom_av3_stackbrowser_2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%%  Opening Function                                                   %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function tom_av3_stackbrowser_2_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for tom_av3_stackbrowser_2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes tom_av3_stackbrowser_2 wait for user response (see UIRESUME)
...
%%  Output Function                                                    %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = tom_av3_stackbrowser_2_OutputFcn(hObject, eventdata, handles) 
...
storage_av3_stackbrowser.quality = 5;
...
if isempty(outfile)
    ...
    outfile = strrep(outfile, '.mat', 'temp.mat');
    save(outfile,'Align');
end
disp(outfile)
...
save(strrep(outalign,'.star','.mat'),'Align')
for w = 1:length(Align)
    Align(w).Filename=strrep(Align(w).Filename, '.em', '');
    Align(w).Filename=strrep(Align(w).Filename, '/', '\/');
end
system(['cp ' instar ' ' outalign])
for x = 1:length(Align)
    system(['sed -i ''''  ''' '/'  Align(x).Filename '/d' ''' ' outalign]);
end

disp(' Star file saved ');
...
function input_av3_stackbrowser_instar_Callback(hObject, eventdata, handles)
% hObject    handle to input_av3_stackbrowser_instar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of input_av3_stackbrowser_instar as text
%        str2double(get(hObject,'String')) returns contents of input_av3_stackbrowser_instar as a double


% --- Executes during object creation, after setting all properties.
function input_av3_stackbrowser_instar_CreateFcn(hObject, eventdata, handles)
% hObject    handle to input_av3_stackbrowser_instar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
pathname = uigetdir('Pick an input star file');
if ischar(pathname)
    set(findobj('Tag','input_av3_stackbrowser_instar'),'String',pathname);
end
