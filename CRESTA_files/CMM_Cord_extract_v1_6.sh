...
a=$( grep '_rlnCoordinateX' $DIR/*.star | cut -d "#" -f 2);
b=$( grep '_rlnCoordinateY' $DIR/*.star | cut -d "#" -f 2);
c=$( grep '_rlnCoordinateZ' $DIR/*.star | cut -d "#" -f 2);
...
then 
	...
	for i in $(seq 1 $length );
		do
		sed "${i}q;d" $OUT/NameCoord.txt | xargs -I{} basename {} _filt.cmm | xargs -I{} grep {} $StarFileDir | awk -v a=$a -v b=$b -v c=$c -F" " '{print $a, $b, $c}' >> $OUT/Orig_coord.txt
			##elif [ "$a" -eq 2 ]
			##then
